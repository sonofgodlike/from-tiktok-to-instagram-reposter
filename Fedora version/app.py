...
def install_deps(log):
    ...
    for pkg in pkgs:
        # Use --user so pip doesn't try to write to the immutable system Python on Bazzite
        result = subprocess.run(
            [sys.executable, "-m", "pip", "install", "--quiet", "--upgrade", "--user", pkg],
            capture_output=True, text=True
        )
        if result.returncode != 0:
            log(f"failed to install {pkg}: {result.stderr.strip()[:120]}")
            return False
        log(f"{pkg.split('==')[0]} ready")

    # Make sure --user bin dir is on PATH so installed scripts are findable
    ...
    # Check ffmpeg — on Bazzite it's usually already present via the base image or
    # can be installed through Homebrew/nix. We won't attempt auto-install since the
    # OS filesystem is immutable; we just tell the user what to do.
    import shutil
    ffmpeg = shutil.which("ffmpeg")
    if ffmpeg:
        log(f"ffmpeg found: {ffmpeg}")
    else:
        log("ffmpeg not found!")
        log("install it with:  brew install ffmpeg")
        log("  or via Distrobox: distrobox-enter -- sudo dnf install ffmpeg")
        log("then relaunch this app.")
        return False
        ...
        def worker():
            try:
                if not install_deps(self.log):
                    self.log("setup failed. see messages above for fix.")
                    return
...
if __name__ == "__main__":
    try:
        ...
        print("On Bazzite, run:  sudo rpm-ostree install python3-tkinter  then reboot.")
        sys.exit(1)
